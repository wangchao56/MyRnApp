...
```typescript
import { HybridWebView } from '@myapp/app';
...
- 如果配置了 `whitelist`，HybridWebView 会在 `onNavigationStateChange` 时校验 URL
- 如果 URL 不在白名单内，会在控制台输出警告
- 可以根据业务需求扩展校验逻辑

## 三、App 主动推送

支持 App 主动向 H5 推送事件，H5 可以通过 `onAppEvent` 监听这些事件。

### App 端发送事件

```typescript
import { bridgeHandlers } from '@myapp/app';
...
```typescript
// 获取当前等待响应的请求数量
console.log('Pending requests:', jsbridge.pendingCount);
```
...
2. **类型安全**：使用 TypeScript 的类型定义确保 API 调用安全
