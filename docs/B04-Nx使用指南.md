...
本项目使�?**Nx** 作为 Monorepo 任务编排工具（替�?Turborepo），配合 **pnpm workspace** 管理依赖�?
...
| 本地缓存 | 未变更的项目跳过重复构建，加�?CI 与本地开�?|
| 依赖可视�?| `nx graph` 查看项目间依赖关�?|
| 按需执行 | 只跑受影响的项目（配�?`--affected`�?|
...
├── nx.json                 # 工作区全局配置
├── apps/
�?  ├── app/project.json    # @myapp/mobile（application�?
...
| 项目�?| 类型 | 路径 | 说明 |
|--------|------|------|------|
| `@myapp/mobile` | application | `apps/mobile` | RN 主应�?|
| `@myapp/web` | application | `apps/web` | 独立 H5 |
| `@myapp/shared` | library | `packages/shared` | 共享业务�?|
...
```bash
# 类型检�?
pnpm nx run @myapp/mobile:typecheck
pnpm nx run @myapp/web:typecheck

# 构建
pnpm nx run @myapp/web:build
pnpm nx run @myapp/jsbridge:build
pnpm nx run @myapp/mobile:build:web

# 开发（长驻进程�?
pnpm nx run @myapp/mobile:dev          # Metro
pnpm nx run @myapp/web:dev          # Webpack :3000
pnpm nx run @myapp/mobile:dev:web      # Rspack :3002
```
...
# 指定多个项目
pnpm nx run-many -t lint --projects=@myapp/mobile,@myapp/web

# 并行启动 dev（app + web�?
pnpm nx run-many -t dev --parallel --projects=@myapp/mobile,@myapp/web
```

### 根目录快捷脚�?

�?`package.json` 已封装常�?Nx 命令，可直接使用�?
...
| `pnpm dev:mobile` | `nx run @myapp/mobile:dev` |
| `pnpm dev:web` | `nx run @myapp/web:dev` |
| `pnpm dev:mobile:web` | `nx run @myapp/mobile:dev:web` |
| `pnpm dev:all` | `nx run-many -t dev --parallel --projects=@myapp/mobile,@myapp/web` |
| `pnpm graph` | `nx graph` |

> **说明**：`pnpm android`、`pnpm ios` 等原�?CLI 命令暂未纳入 Nx target，仍由根脚本直接调用 React Native CLI�?
...
- `@myapp/mobile` �?依赖 `@myapp/shared`、`@myapp/jsbridge`
...
每个项目�?`project.json` 声明 **targets**，通过 `nx:run-script` 执行对应 `package.json` 中的 npm script�?
...
- `^build`：先构建所�?*依赖项目**�?`build` target  
  例如构建 `@myapp/web` 前会先构�?`@myapp/jsbridge`
...
3. **确认 workspace 已包�?*（`pnpm-workspace.yaml` 已有 `apps/*`，无需改）

4. **验证**

```bash
pnpm install
pnpm nx show projects   # 应出�?@myapp/windows
pnpm nx run @myapp/windows:typecheck
```

5. **（可选）�?package.json 添加快捷脚本**
...
CI 环境可开�?Nx Cloud 进一步共享缓存（可选，需单独配置）�?

## �?pnpm 的配�?

```bash
# 在特定包安装依赖（与 Nx 无关，仍�?pnpm filter�?
pnpm --filter @myapp/mobile add some-package
...
依赖安装�?[B01-安装命令](./B01-安装命令.md) �?[install-npm-packages 技能](../.claude/skills/install-npm-packages/SKILL.md)�?

## 故障排除

### `tsc` 找不到（Windows�?

�?`nx run typecheck` �?`Cannot find module '.../typescript/bin/tsc'`�?
...
确认存在 `project.json` �?`name` �?`package.json` �?`name` 一致，然后�?
...
pnpm nx show project @myapp/web --web   # 浏览器查�?
```
