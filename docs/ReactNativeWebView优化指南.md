...
本文档总结了对项目�?`react-native-webview` 组件的全面优化工作，包括性能提升、错误处理增强、JS 注入优化等内容�?
## 目录
...
| 优化�?| 说明 |
|--------|------|
| 硬件加�?| `androidLayerType="hardware"` 开启硬件加�?|
| 缓存优化 | `cacheEnabled` 启用缓存 |
| Cookie 支持 | `thirdPartyCookiesEnabled` 启用第三�?Cookie（Android�?|
| 自适应缩放 | `scalesPageToFit` 优化页面显示 |

### 2. JS 注入优化

- **注入时机改进**：使�?`injectedJavaScriptBeforeContentLoaded` 在内容加载前注入
- **就绪事件**：新�?`hybridWebViewReady` 自定义事�?- **消息队列**：实现消息队列机制，避免�?WebView 准备前发送消息失�?
### 3. 错误处理增强

- **HTTP 错误处理**：新�?`onHttpError` 处理 HTTP 状态码
...
- **等待就绪**：新�?`waitForBridgeReady()` 函数
- **状态检�?*：新�?`isBridgeReady()` 函数
- **消息队列 TTL**：消�?TTL 机制，防止内存泄�?- **回调清理**：自动清理过期回�?
### 5. 功能增强

- **扩展 ref 接口**�?  - `goBack()` / `goForward()` 导航
  - `clearCache()` 清除缓存
  - `clearHistory()` 清除历史
- **配置�?*�?  - `enableDebug` 调试日志控制
  - `maxRetryCount` 重试次数控制
- **默认组件**：内�?Loading �?Error 组件
...
| `whitelist` | string[] | [] | URL 白名�?|
| `enableDebug` | boolean | __DEV__ | 是否启用调试日志 |
| `maxRetryCount` | number | 3 | 最大重试次�?|
| `renderLoading` | () => ReactNode | 默认加载组件 | 自定义加载组�?|
| `renderError` | (error) => ReactNode | 默认错误组件 | 自定义错误组�?|
...
### 1. apps/mobile/src/components/HybridWebView/index.tsx

主要�?WebView 封装组件，包含所有优化逻辑�?
**核心功能�?*
- WebView 配置和性能优化
- 桥接消息处理
- 错误处理和重试机�?- 默认 UI 组件

**相关代码�?* [HybridWebView 组件](file:///e:/workspace/MyRnApp/apps/mobile/src/components/HybridWebView/index.tsx)
