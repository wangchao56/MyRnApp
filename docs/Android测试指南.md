...
### 1. 安装依赖
已成功安装 `@react-native-camera-roll/camera-roll@^7.10.2` 到根目录依赖中。
...
### 3. 创建测试页面
已创建专门的测试页面 `MediaSaveTestScreen`，可通过底部导航的 "Media" 标签页访问。

### 4. 更新的 Hook
`useSaveMedia` hook 已更新为：
- 跨平台兼容（Web + Native）
- 自动处理 Android 权限请求
- 支持 Android 不同版本的权限差异
...
### Q: 权限请求没有弹出？
A: 确保在 `AndroidManifest.xml` 中添加了相应的权限声明。

### Q: 保存失败，提示权限被拒绝？
A: 
- 检查是否正确添加了所有必需的权限
- Android 13+ 需要 `READ_MEDIA_IMAGES` 和 `READ_MEDIA_VIDEO`
- Android 10 以下需要 `READ_EXTERNAL_STORAGE` 和 `WRITE_EXTERNAL_STORAGE`

### Q: 应用崩溃或报错？
A:
- 确保 Metro bundler 正在运行
- 检查终端中的错误日志
- 确保 `@react-native-camera-roll/camera-roll` 已正确安装
