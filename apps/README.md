...
| 目录 | 包名 | 平台 | 说明 |
|------|------|------|------|
| [app/](./mobile/) | `@myapp/mobile` | Android / iOS / RNW | React Native 主应�?|
| [web/](./web/) | `@myapp/web` | Web (H5) | 独立 H5 站点，Bridge 联调 |

## 规划中的平台应用

以下目录将在对应平台接入时创建，共享 `@myapp/shared` �?`@myapp/jsbridge`�?
...
> 原生工程（`android/`、`ios/`）目前仍在仓库根目录，后续可迁入 `apps/mobile/` 以统一多平台结构�?

## Nx 命令

```bash
nx run @myapp/mobile:typecheck
