...
```ts
import NativeShare, { shareToQQ } from '@myapp/nativeshare';

const ns = new NativeShare({ wechatConfig: { /* ... */ } });
ns.setShareData({ title: '标题', desc: '描述', link: location.href, icon: '' });
ns.call('wechatfriend');
```

由 `@myapp/share` 在 Web 端通过 workspace 依赖引入。
